...
base_npy_dir = os.path.join(base_drive, "simulated-AgMERRA/CMC_AgMERRA_0125_annual_npy_20210115")
base_sim_npy_dir = os.path.join(base_drive, "simulated_yield_wwh/weather_daily_npy_0125_C360_T0_W0_N200_A0")

output_dir = os.path.join(base_drive, "simulated-AgMERRA/model_input")
...
def export_snowfall(output_filename, fdd_filepattern, gdd_filepattern):
    ...
    for year in tqdm.tqdm(range(1999, 2010 + 1), desc=output_filename):
        ...
        for sim_model_name, year_shift in simulation_models:
            sim_wheat = np.load(os.path.join(base_sim_npy_dir,
                                             "{}/yield_wwh_{}_ann_{}.npy".format(sim_model_name.lower(),
                                                                                 sim_model_name.lower(),
                                                                                 year - year_shift)))  # simulated yield is shifted 1980 is 1981's yield.
            # if sim_model_name == "lpj-guess" or sim_model_name == "apsim-ugoe":
            #     sim_wheat = np.load(os.path.join(base_sim_npy_dir,
            #                                      "{}_agmerra/yield_wwh_{}_agmerra_ann_{}.npy".format(sim_model_name,
            #                                                                                          sim_model_name,
            #                                                                                          year)))
            # else:
            #     # simulated yield is shifted 1980 is 1981's yield.
            #     sim_wheat = np.load(os.path.join(base_sim_npy_dir, "{}_agmerra/yield_wwh_{}_agmerra_ann_{}.npy".format(sim_model_name, sim_model_name, year - 1)))  # simulated yield is shifted 1980 is 1981's yield.
            sim_wheat[sim_wheat == -9999] = np.nan
            sim_wheat[sim_wheat < -9999] = np.nan
            sim_wheat[sim_wheat == 0] = np.nan
            simulated_npy.append(sim_wheat)
        ...
        df_fdd_gdd_raw = pd.DataFrame({
            "county_fips": fips_array_flatten,
            "yield_sim_n200_EPIC-TAMU": simulated_npy[0].flatten(),
            "yield_sim_n200_GEPIC": simulated_npy[1].flatten(),
            "yield_sim_n200_LPJ-GUESS": simulated_npy[2].flatten(),
            "yield_sim_n200_LPJmL": simulated_npy[3].flatten(),
            "yield_sim_n200_pDSSAT": simulated_npy[4].flatten(),
            "yield_sim_n200_APSIM-UGOE": simulated_npy[5].flatten(),
            "yield_sim_n200_EPIC-IIASA": simulated_npy[6].flatten(),
            "yield_sim_n200_PEPIC": simulated_npy[7].flatten(),
            # "yield_sim_n200_LPJmL_without_shift": simulated_npy[8].flatten(),
            # "yield_sim_n200_PEPIC_without_shift": simulated_npy[9].flatten(),
            "fdd1": weather_dataset[0].flatten(),
            "fdd2": weather_dataset[1].flatten(),
            "fdd3": weather_dataset[2].flatten(),
            "fdd1_sc2": weather_dataset[3].flatten(),
...
if __name__ == "__main__":
    ...
    export_snowfall(
        os.path.join(output_dir, "winter_wheat_input_AgMERRA_ifdd_igdd_v20210122_0125_N200.csv"),
