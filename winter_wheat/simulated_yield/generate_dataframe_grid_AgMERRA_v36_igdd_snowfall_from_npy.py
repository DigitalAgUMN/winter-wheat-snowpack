...
base_sim_npy_dir = os.path.join(base_drive, "simulated_yield_wwh/weather_daily_npy_0125")
...
def export_snowfall(output_filename, fdd_filepattern, gdd_filepattern):
    ...
    for year in tqdm.tqdm(range(1999, 2010 + 1), desc=output_filename):
        ...
        simulation_models = ("epic-tamu", "gepic", "lpj-guess", "lpjml", "pdssat", "apsim-ugoe")

        simulated_npy = []

        for sim_model_name in simulation_models:
        ...
        df_fdd_gdd_raw = pd.DataFrame({
            "county_fips": fips_array_flatten,
            "yield_sim_epic-tamu": simulated_npy[0].flatten(),
            "yield_sim_gepic": simulated_npy[1].flatten(),
            "yield_sim_lpj-guess": simulated_npy[2].flatten(),
            "yield_sim_lpjml": simulated_npy[3].flatten(),
            "yield_sim_pdssat": simulated_npy[4].flatten(),
            "yield_sim_apsim-ugoe": simulated_npy[5].flatten(),
            "fdd1": weather_dataset[0].flatten(),
            "fdd2": weather_dataset[1].flatten(),
            "fdd3": weather_dataset[2].flatten(),
            "fdd1_sc2": weather_dataset[3].flatten(),
...
if __name__ == "__main__":
    ...
    export_snowfall(
        os.path.join(output_dir, "winter_wheat_input_AgMERRA_ifdd_igdd_v20210122_0125.csv"),
