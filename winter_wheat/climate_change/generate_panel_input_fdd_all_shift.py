...
def generate_panel_output(output_filename, df_all, shift_days=0):
    ...
    df_all["monthday"] = df_all["month"] * 100 + df_all["day"]
    df_all[df_all["county_fips"]==1001].to_csv(output_filename.replace(".csv", "_test_input.csv"), index=False)

    print(532 + shift_days)
    print(df_all[(df_all["county_fips"]==1001) & (df_all["month"]==2)])
    if shift_days < 0:
        df_all = df_all[((df_all["monthday"] >= (832 + shift_days)) | (df_all["monthday"] < (532 + shift_days))) & (
                    df_all["season_year"] >= 1980)]
    else:
        df_all = df_all[((df_all["monthday"] >= (901 + shift_days)) | (df_all["monthday"] < (601 + shift_days))) & (
                df_all["season_year"] >= 1980)]
    print(df_all[(df_all["county_fips"]==1001) & (df_all["month"]==2)])

    df_all = df_all.copy()
    print(df_all[df_all["county_fips"]==1001])
    print(df_all.dtypes, shift_days)
    ...
    df_all[df_all["county_fips"]==1001].to_csv(output_filename.replace(".csv", "_test.csv"), index=False)
    ...
    df_all = df_all.groupby(["scenario", "county_fips", "season_year"]).agg({
        ...
        # 'fdd3': 'sum',
        "fdd1_sc2": 'sum',
        # "fdd1_sc5": 'sum', "fdd1_sc10": 'sum', "fdd1_sc15": 'sum', "fdd1_sc20": 'sum',
    ...
    df_all = df_all[
        [
            ...
            # "fdd2", "fdd3",
            "fdd1_sc2",
            # "fdd1_sc5", "fdd1_sc10", "fdd1_sc15", "fdd1_sc20",
            # "fdd2_sc2", "fdd2_sc5", "fdd2_sc10", "fdd2_sc15", "fdd2_sc20",
            # "fdd3_sc2", "fdd3_sc5", "fdd3_sc10", "fdd3_sc15", "fdd3_sc20",
            "fdd1_day",
            # "fdd2_day", "fdd3_day",
            "fdd1_sc2_sctf",
            # "fdd1_sc5_sctf", "fdd1_sc10_sctf", "fdd1_sc15_sctf", "fdd1_sc20_sctf",
...
def process_annual_fdd(weather_filename, swe_filename, output_filename, other_weather_file=None,
    ...
    if weather_august_filename is not None:
        df_nex_other = read_nex_gddp(weather_august_filename)
        df_nex = pd.concat([df_nex, df_nex_other], ignore_index=True)
    if weather_june_filename is not None:
        df_nex_other = read_nex_gddp(weather_june_filename)
        df_nex = pd.concat([df_nex, df_nex_other], ignore_index=True)
    ...
    generate_panel_output(output_filename, df_all, shift_days)


def process_annual_fdd_with_corrected_swe(weather_filename, swe_filename, reference_data, cchist_data, output_filename,
                                          swe_corrected_filename, other_weather_file=None, weather_august_filename=None,
...
def get_bias_correction(df_cc, df_ref):
    ...
    target_data = df_cc[((df_cc["date"] >= "1998-09-01") & (df_cc["date"] < "2006-01-01") & (df_cc["scenario"] == "historical") & ((df_cc["month"] < 6) | (df_cc["month"] >= 9)))]
...
def process(model_name):
    ...
    base_dir_swe = os.path.join(os.path.join(base_drive, "CMIP5_snw_county_shift"), model_name)
    # base_dir_swe_cmip5 = os.path.join(os.path.join(base_drive, "CMIP5_snw_county_shift"), model_name)
    ...
    for scenario in scenarios:
        for year in range(periods[scenario][0], periods[scenario][1] + 1):
            ...
            weather_august_filename = os.path.join(base_dir_nex,
                                            "NEX-GDDP-all-august-w25000-{}-{}-{}.csv".format(model_name, scenario, year))
            weather_june_filename = os.path.join(base_dir_nex,
                                            "NEX-GDDP-all-june-w25000-{}-{}-{}.csv".format(model_name, scenario, year))
            swe_filename = os.path.join(base_dir_swe,
                                            "df-snw-{}-{}-{}.csv".format(model_name, scenario, year))

            output_filename = os.path.join(output_dir,
                                           "climate_input_data_fdd_{}_{}_{}_shift-15_20210903.csv".format(model_name, scenario,
                                                                                                 year))
            ...
            # results.append(pool.apply_async(process_annual_fdd, args=(
            #     weather_filename, swe_filename, output_filename, weather_other_filename,
            #     weather_august_filename, weather_june_filename, -15)))


    output_dir_swe = os.path.join(os.path.join(base_drive, "CMIP5_snw_county_corrected-shift"), model_name)
    output_dir = os.path.join(os.path.join(base_drive, "NEX-GDDP-annual-swe-corrected-shift"), model_name)
    make_dirs(output_dir)
    ...
    for scenario in scenarios:
        for year in range(periods[scenario][0], periods[scenario][1] + 1):
            ...
            swe_filename = os.path.join(base_dir_swe,
                                            "df-snw-{}-{}-{}.csv".format(model_name, scenario, year))
            output_filename = os.path.join(output_dir,
                                           "climate_input_data_fdd_swe_corrected_{}_{}_{}_shift-15_20210903.csv".format(
                                               model_name, scenario,
                                               year))
            swe_corrected_filename = os.path.join(output_dir_swe,
                                                  "df-snw-corrected-{}-{}-{}_shift-15.csv".format(model_name, scenario, year))
            if not os.path.exists(weather_filename):
                continue
            if not os.path.exists(weather_other_filename):
                weather_other_filename = None
            ...
            results.append(pool.apply_async(process_annual_fdd_with_corrected_swe,
                                            args=(weather_filename, swe_filename, reference_data, cchist_data,
                                                  output_filename, swe_corrected_filename, weather_other_filename,
                                                  weather_august_filename, weather_june_filename, -15)))
