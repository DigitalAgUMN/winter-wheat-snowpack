...
def draw_bar_plot_three_pane(base_dir, output_filename, models, variables, agmip_models, filename_pattern, five_state=False, sharedy=False, output_csv_file=None, color_palette=None):
    ...
    fig, axes_all = plt.subplots(3, 2, figsize=(12, 10), gridspec_kw={"height_ratios": [1.3, 1, 1]}, dpi=300, constrained_layout=False)
...
def draw_subplot_for_agmip(bar_chart_data, raw_data, ax=None, selected_variables=None, obs_color=None):
    ...
    for ii, group_data in enumerate(plotter.plot_data):
        for jj, hue_level in enumerate(plotter.hue_names):
            ...
            # print(var_name, np.sqrt(variance) / np.abs(estimate))
    ...
    for kk, hue_level in enumerate(plotter.hue_names):
        ...
        if plotter.confint.size:
            ...
            confint = plotter.confint[:, kk]
            errcolors = [plotter.errcolor] * len(offpos)
            # plotter.draw_confints(axes[i, j],
            #                    offpos,
            #                    confint,
            #                    errcolors,
            #                    plotter.errwidth,
    ...
    # add AgMIP average coefficient
    y_est = agmip_coeff[selected_variables[0]][0]
    y_err = agmip_coeff[selected_variables[0]][1] * 1.96 * 0.001
    agmip_color = "#DC0000"  # "#8491B4"

    ax.plot(x, [y_est, y_est], '--', color=agmip_color)
    ax.fill_between(x, y_est - y_err, y_est + y_err, alpha=0.2, color=agmip_color, zorder=2)
...
def draw_barchart_together(color_palette=None):
    ...
    output_dir = "N:/WinterWheat/output/manuscript_20211015_9GCMs"

    make_dirs(output_dir)

    output_file = os.path.join(output_dir, "FigureS07_barplot-coeff-avg-errorbar-with-AgMIPs-avg.png")
    output_csv_file = os.path.join(output_dir, "Figure07_barplot-coeff-avg-errorbar-with-AgMIPs-avg.csv")
...
def draw_lineplot(output_filename, color_palette=None):
    ...
    p = axes.plot((0, 1),
                  ...
                  lw=6, label="Observation", c=color_palette[-1], alpha=.7)
...
def read_agmip_coeff():
    global agmip_coeff
    agmip_coeff = {}

    filename = get_project_root() / "input/AgMIP/coef_agmip_ensemble.csv"
    with open(filename, "r") as f:
        csv_reader = csv.reader(f)
        next(csv_reader)
        for row in csv_reader:
            agmip_coeff[row[0]] = (float(row[1]), float(row[2]))
    print(agmip_coeff)
