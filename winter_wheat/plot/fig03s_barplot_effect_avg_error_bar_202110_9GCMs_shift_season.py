...
field_names = {
    ...
    "gdd_autumn": ("effect", "GDD$_{fall}$",),
    "gdd_winter": ("effect", "GDD$_{winter}$",),
    "gdd_spring": ("effect", "GDD$_{spring}$",),
    "gdd_all": ("effect", "GDD",),
...
def draw_bar_plot_only_avg(base_dir, output_filename, models, variables, filename_pattern, five_state=False, sharedy=False, output_csv_file=None, color_pallete=None):
    ...
    fig, axes = plt.subplots(1, 3, figsize=(12.5, 4), gridspec_kw={"width_ratios": [2.6, 3.6, 2.6], "wspace": 0}, sharey=True, dpi=300, constrained_layout=False)

    plt.subplots_adjust(left=0.10, bottom=0.11, right=0.98, top=0.95, wspace=0.14, hspace=0.18)
            ...
            for model_name in models:
                for period_name in periods:
                    for each_var_name in variables[var_name]:
                        ...
                        print(",".join(map(str, [model_print_name, scenario, each_var_name, coeff[each_var_name].value, hist_mean, future_mean])))
    ...
    draw_subplot_for_avg(bar_chart_data, raw_data, ax=axes[0], selected_variables=["fdd1", "fdd1_sc2_sctf"])
    draw_subplot_for_avg(bar_chart_data, raw_data, ax=axes[1], selected_variables=["gdd_autumn", "gdd_winter", "gdd_spring"])
    draw_subplot_for_avg(bar_chart_data, raw_data, ax=axes[2], selected_variables=["prcp_all", "snowfall_all"])
    ...
    for i in range(3):
        ...
        axes[i].set_ylim([-38, 15])
        axes[i].set_yticks([-30, -20, -10, 0, 10])
        if "shift+15" in output_filename:
            axes[i].set_yticks([-30, -20, -10, 0, 10])
        if "shift-15" in output_filename:
            axes[i].set_yticks([-30, -20, -10, 0, 10])
...
def draw_new_figures():
    ...
    base_dir = "N:/WinterWheat/county_map/v20210911-shift+15"  # add more GCMs

    output_dir = "N:/WinterWheat/output/manuscript_20211001_9GCMs_season_Fig3"

    make_dirs(output_dir)
    output_file = os.path.join(output_dir, "Figure03s_barplot-stress-effect-avg-errorbar-shift+15.png")
    output_csv_file = os.path.join(output_dir, "Figure03s_barplot-stress-effect-avg-errorbar-shift+15.csv")
    ...
    output_file = os.path.join(output_dir, "Figure03s_barplot-stress-effect-avg-errorbar.png")
    output_csv_file = os.path.join(output_dir, "Figure03s_barplot-stress-effect-avg-errorbar.csv")
    draw_bar_plot_only_avg(base_dir, output_file, models, variables, "gridMET_{}_avg.csv", output_csv_file=output_csv_file)
