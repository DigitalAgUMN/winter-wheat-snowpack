...
def process_bc(model_name):
    ...
    base_gdd_dir = os.path.join("N:/WinterWheat/NEX-GDDP-annual", model_name)

    output_base_dir = "N:/WinterWheat/model-input-swe-bc"
    output_dir = os.path.join(output_base_dir, model_name)
    make_dirs(output_dir)

    output_filename = os.path.join(output_dir, "winter_wheat_input_igdd_sdepth_bc_v20210111_{}.csv".format(model_name))

    if os.path.exists(output_filename):
        return

    df = export(base_dir, base_gdd_dir, model_name,
                fdd_filename_pattern="climate_input_data_fdd_swe_corrected_{}_{}_{}_20210109.csv",
                gdd_filename_pattern="climate_input_data_igdd_{}_{}_{}_20210109.csv",
                )
...
def process(model_name):
    ...
    output_filename = os.path.join(output_dir, "winter_wheat_input_igdd_v20200927_{}.csv".format(model_name))

    if os.path.exists(output_filename):
        return

    df = export(base_dir, base_dir, model_name,
                fdd_filename_pattern="climate_input_data_fdd_{}_{}_{}_20210109.csv",
                gdd_filename_pattern="climate_input_data_igdd_{}_{}_{}_20210109.csv",
                )
    if df is not None:
        df.to_csv(output_filename, index=False)
